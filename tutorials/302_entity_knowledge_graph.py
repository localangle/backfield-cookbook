...
def _(mo):
    ...
    4. Explore the graph visually and answer a few graph-shaped questions.
...
def _(mo):
    mo.md(r"""
    ## Step 5: Find hubs and brokers

    A graph makes structural questions straightforward:

    - **Degree** counts an entity's direct neighbors.
    - **Relationship diversity** counts the distinct natures on those edges.
    - **Betweenness centrality** measures how often a node lies on shortest paths between other nodes. A high score suggests a broker connecting otherwise separate parts of the network.

    These are graph properties, not measures of editorial importance. The result also depends on the political cohort and its one-hop context.
    """)
    return


@app.cell
def _(entity_graph, mo, nx):
    connected_graph = entity_graph.subgraph(
        [node for node in entity_graph if entity_graph.degree(node) > 0]
    ).copy()
    betweenness = nx.betweenness_centrality(connected_graph)

    graph_leaders = []
    for _node, _data in connected_graph.nodes(data=True):
        _natures = {
            nature
            for _, edge in connected_graph[_node].items()
            for nature in edge["natures"]
        }
        graph_leaders.append(
            {
                "label": _data["label"],
                "node_type": _data["person_type"] or _data["entity_type"],
                "degree": connected_graph.degree(_node),
                "nature_count": len(_natures),
                "betweenness": betweenness[_node],
            }
        )

    _top_hubs = sorted(
        graph_leaders,
        key=lambda row: (-row["degree"], -row["nature_count"], row["label"]),
    )[:10]
    _top_brokers = sorted(
        graph_leaders,
        key=lambda row: (-row["betweenness"], -row["degree"], row["label"]),
    )[:10]

    _hub_rows = [
        "| Entity | Type | Neighbors | Relationship natures |",
        "|---|---|---:|---:|",
    ]
    _hub_rows.extend(
        f"| {row['label']} | {row['node_type'].replace('_', ' ')} | "
        f"{row['degree']} | "
        f"{row['nature_count']} |"
        for row in _top_hubs
    )
    _broker_rows = [
        "| Entity | Type | Betweenness | Neighbors |",
        "|---|---|---:|---:|",
    ]
    _broker_rows.extend(
        f"| {row['label']} | {row['node_type'].replace('_', ' ')} | "
        f"{row['betweenness']:.3f} | {row['degree']} |"
        for row in _top_brokers
    )
    ...
    return connected_graph, graph_leaders


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    ## Step 6: Find a relationship path

    A shortest-path query can explain how two records are connected even when there is no direct edge. Choose two connected entities. Each hop reports every nature collapsed onto that edge.

    Because the graph is undirected, the path answers “how are these records connected?” The original direction of each Stylebook relationship is still available in the edge's `relationships` attribute if an application needs it.
    """)
    return


@app.cell
def _(connected_graph, mo, nx):
    _largest_component = (
    ...
    _path_nodes = sorted(
        _largest_component,
        key=lambda node: connected_graph.nodes[node]["label"].lower(),
    )

    def _node_type_label(node):
        data = connected_graph.nodes[node]
        return (data["person_type"] or data["entity_type"]).replace("_", " ")

    path_label_to_node = {
        (
            f"{connected_graph.nodes[node]['label']} "
            f"({_node_type_label(node)})"
        ): node
        for node in _path_nodes
    }
    _path_labels = list(path_label_to_node)

    mo.stop(
        len(_path_labels) < 2,
        "The selected graph does not contain a component with two entities.",
    )

    path_start = mo.ui.dropdown(
        options=_path_labels,
        value=_path_labels[0],
        label="Start entity",
    )
    path_end = mo.ui.dropdown(
        options=_path_labels,
        value=_path_labels[-1],
        label="End entity",
    )
    mo.hstack([path_start, path_end], justify="start", gap=1.5)
    return path_end, path_label_to_node, path_start
...
def _(
    ...
    _start_node = path_label_to_node[path_start.value]
    _end_node = path_label_to_node[path_end.value]
    _path = nx.shortest_path(connected_graph, _start_node, _end_node)

    _path_rows = []
    for _left, _right in zip(_path, _path[1:]):
        _edge = connected_graph.edges[_left, _right]
        _left_label = connected_graph.nodes[_left]["label"]
        _right_label = connected_graph.nodes[_right]["label"]
        _natures = ", ".join(_edge["natures"]) or "unspecified"
        _path_rows.append(
            f"1. **{_left_label}** → **{_right_label}**  \n"
            f"   `{_natures}` ({_edge['relationship_count']} connection record(s))"
        )

    mo.md(
        f"**Shortest path: {len(_path) - 1} hop(s)**\n\n"
        + "\n".join(_path_rows)
    )
    return
...
def _(mo):
    mo.md(r"""
    ## Step 7: Who connects politics to institutions and places?

    Organization and location neighbors give the political network useful context. This query finds the people with the most direct links to those institutions and places, then reports the relationship natures behind the links.
    """)
    return


@app.cell
def _(connected_graph, mo):
    context_connectors = []
    for _node, _data in connected_graph.nodes(data=True):
        if _data["entity_type"] != "person":
            continue

        _context_neighbors = [
            neighbor
            for neighbor in connected_graph.neighbors(_node)
            if connected_graph.nodes[neighbor]["entity_type"]
            in {"organization", "location"}
        ]
        _natures = {
            nature
            for neighbor in _context_neighbors
            for nature in connected_graph.edges[_node, neighbor]["natures"]
        }
        _organizations = sum(
            connected_graph.nodes[neighbor]["entity_type"] == "organization"
            for neighbor in _context_neighbors
        )
        _locations = sum(
            connected_graph.nodes[neighbor]["entity_type"] == "location"
            for neighbor in _context_neighbors
        )
        context_connectors.append(
            {
                "label": _data["label"],
                "person_type": _data["person_type"],
                "organizations": _organizations,
                "locations": _locations,
                "context_neighbors": len(_context_neighbors),
                "natures": sorted(_natures),
            }
        )

    context_connectors.sort(
        key=lambda row: (-row["context_neighbors"], row["label"].lower())
    )
    _rows = [
        "| Person | Type | Organizations | Locations | Natures |",
        "|---|---|---:|---:|---|",
    ]
    _rows.extend(
        f"| {row['label']} | {row['person_type'].replace('_', ' ')} | "
        f"{row['organizations']} | {row['locations']} | "
        f"{', '.join(row['natures']) or '—'} |"
        for row in context_connectors[:10]
        if row["context_neighbors"] > 0
    )

    mo.md("\n".join(_rows))
    return (context_connectors,)
...
def _(mo):
    ...
    Try changing `PERSON_TYPES`, hiding one kind of context node, showing the unconnected records, filtering to one relationship nature, or choosing two distant entities in the path explorer. A production application could cache the graph, let editors inspect directional relationship details, or combine these graph features with search and recommendation ranking.
